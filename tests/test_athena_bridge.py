...
from thero.integrations.athena_bridge import (
    ATHENA_PATH_ENV_VAR,
    find_athena_script,
    run_athena_index,
)
...
def test_env_var_takes_priority_when_file_exists(tmp_path, monkeypatch):
    ...
    result = find_athena_script(tmp_path / "thero" / "thero.py")
...
def test_falls_back_to_ensure_tool_repo_when_no_sibling(tmp_path, monkeypatch):
    ...
    with patch(
        "thero.integrations.athena_bridge.ensure_tool_repo",
...
def test_run_athena_index_returns_false_when_script_not_found(tmp_path):
    with patch(
        ...
        return_value=None,
    ), patch("thero.integrations.athena_bridge.run_command") as mock_run:
        result = run_athena_index(tmp_path / "thero.py")

    assert result is False
    mock_run.assert_not_called()


def test_run_athena_index_propagates_success_returncode(tmp_path):
    with patch(
        "thero.integrations.athena_bridge.find_athena_script",
        return_value=tmp_path / "athena.py",
    ), patch(
        "thero.integrations.athena_bridge.run_command",
