...
def test_git_missing_returns_none_without_trying_anything(tmp_path):
    with patch(
        "thero.integrations.tool_repo.command_exists", return_value=False
    ), patch("thero.integrations.tool_repo.run_command") as mock_run, patch(
        "thero.integrations.tool_repo.confirm"
    ) as mock_confirm:
...
def test_missing_dir_confirmed_clones_and_returns_entry_path(tmp_path):
    ...
    with patch(
        "thero.integrations.tool_repo.command_exists", return_value=True
    ), patch(
        "thero.integrations.tool_repo.confirm", return_value=True
    ), patch(
        "thero.integrations.tool_repo.run_command", side_effect=fake_clone
    ) as mock_run:
        result = ensure_tool_repo(
            "athena", "https://example.com/athena.git", "athena.py",
...
def test_missing_dir_declined_returns_none_without_cloning(tmp_path):
    ...
    with patch(
        ...
        "thero.integrations.tool_repo.confirm", return_value=False
    ), patch("thero.integrations.tool_repo.run_command") as mock_run:
        result = ensure_tool_repo(
            "athena", "https://example.com/athena.git", "athena.py",
            tools_root=tools_root,
        )
...
def test_clone_failure_returns_none(tmp_path):
    ...
    with patch(
        ...
        "thero.integrations.tool_repo.run_command",
...
def test_existing_dir_same_hash_does_not_pull(tmp_path):
    ...
    with patch(
        ...
        "thero.integrations.tool_repo.run_command", side_effect=fake_run
    ) as mock_run, patch(
        "thero.integrations.tool_repo.confirm"
    ) as mock_confirm:
        result = ensure_tool_repo(
